...
| `crates/biliup-observability/src/sqlite.rs` | 独立 SQLite migrations 与每 Runtime 私有连接；共享库中的 writer 按运行身份注册、续租、关闭和一次性标记心跳中断，事件/附件仍走幂等事务，并保留只读游标查询、保留/WAL/低盘保护和一致性备份。查询支持级别/分类的精确集合与 `newest_first` 倒序，集合大小在 `push_filters` 里统一设界，`count` 与分页受同一约束。 | `SqliteStore`、`StoreOptions`、`Repository`、`Query`、`MIGRATOR`、`touch_writer`、`reap_stale_writers`、`push_filters` |
