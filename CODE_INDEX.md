...
| `crates/biliup/src/downloader/live/bilibili.rs` | B 站直播提取与选流：解析真实房间信息、API 或 master m3u8 候选，按配置选 CDN，并可在录制前探测失败后回退同协议候选；每个私有候选已携带平台实际 `qn`。 | `Bilibili`、`BilibiliLive::check_stream`、`get_stream_candidates`、`select_stream_url`、`BiliStreamCandidate`、`parse_codec_urls`、`parse_master_m3u8` |
