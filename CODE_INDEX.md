...
| `crates/biliup-cli/src/server/common/upload.rs` | 编排直播分段上传、会话级幂等投稿协调、缺失补传、attempt lease/watchdog（分阶段计时 + 传输阶段滑窗判速）、线路决策入口以及远端结果落库；滑窗判慢中止使用独立慢速失败类别，聚合诊断只承诺最近确认分块；两个服务端入口共用预上传瞬时网络重试，分段成功会在事务外唤醒有 durable 投稿意图的父会话。投稿明确失败的退避基数 60 s 与 `PeriodicScan` 周期对齐；错误串含 `code: 21566`（账号级投稿频控）走独立的 15 min 起、4 h 封顶曲线，且上一条 `last_submit_error` 也是频控时不再重复告警。 | `process_with_upload`、`reconcile_session_submission`、`spawn_session_submission`、`persist_segment`、`decide_upload_line`、`pre_upload_with_retry`、`retry_pre_upload_requests`、`upload_enrolled_with_watchdog`、`claim_manual_recovery`、`run_claimed_recovery`、`stop_missing_segment_attempt`、`segment_part_title` |
...
| `crates/biliup-cli/src/server/common/upload_session.rs` | 维护投稿会话恢复（场次键优先、时钟窗口兜底）、单调持久投稿意图与退避时间，并以生命周期账本检查完整性、确定性重建分 P 和原子 claim/finalize；下播后 `BLOCKED_RECHECK_INTERVAL` 内仅有在飞行（pending/uploading）的阻塞判为 quiet，不写 `blocked_signature`；关闭态零基线会话在同一事务中逻辑终结为 `discarded_empty`。 | `SessionCompleteness`、`request_session_submit`、`session_submit_readiness`、`discard_empty_session`、`schedule_submit_retry`、`hold_session_submit`、`select_recovery_candidate`、`reusable_streamer_info`、`touch_session_activity`、`claim_complete_session`、`BLOCKED_RECHECK_INTERVAL` |
