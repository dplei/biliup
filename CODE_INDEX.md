...
| `crates/biliup-cli/src/server/common/download.rs` | 录制主流程与分段事件处理：拉流、断线重试、切片校验，把有效分段登记后交给上传管道，并在尾段 durable enrollment 后持久关闭会话投稿意图；校验淘汰统一经过 `remove_invalid_segment`，当前该边界只接收路径与自由原因并异步删除。 | `start_download_workflow`、`DownloadTask`、`SegmentEventProcessor`、`remove_invalid_segment`、`persist_closed_session_intents` |
