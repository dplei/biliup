...
| `crates/biliup-cli/src/server/common/download.rs` | 录制主流程与分段事件处理：拉流、断线重试、切片校验，把有效分段登记后交给上传管道，并在尾段 durable enrollment 后持久关闭会话投稿意图；校验淘汰统一经过同步等待的 `remove_invalid_segment`，删除成功后携带录制/分段身份发出原生丢弃事件。 | `start_download_workflow`、`DownloadTask`、`SegmentEventProcessor`、`remove_invalid_segment`、`persist_closed_session_intents` |
| `crates/biliup-cli/src/server/common/route_health.rs` | 每场录制的拉流线路状态机：按签名无关的 `RouteKey` 累计失败、认证刷新、冷却与稳定尝试，并从平台刷新后的候选中轮换健康线路；是否启用切线由调用端传入。 | `RouteKey`、`RouteHealthState`、`observe_live_attempt`、`select_route`、`RouteSelection` |
