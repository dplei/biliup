...
| `crates/biliup-cli/src/server/common/route_health.rs` | 每场录制的拉流线路状态机：按签名无关的 `RouteKey` 累计失败、认证刷新、冷却与稳定尝试；稳定或有媒体产出的 attempt 会复位旧失败串，但只有原稳定判据进入稳定指标；状态机再从平台刷新后的候选中轮换健康线路，是否启用切线由调用端传入。 | `RouteKey`、`RouteHealthState`、`observe_live_attempt`、`select_route`、`RouteSelection` |
