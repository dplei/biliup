...
- `crates/biliup-cli/src/server/common/download.rs` → `crates/biliup-cli/src/observe.rs`（`RecordingIdentity`、`segment_enrolled`、`segment_discarded`）：录制循环与分段处理器共用同一身份对象，登记结果逐一映射为原生事件；校验删除成功也在同一等待路径携带 R/S/DA、大小、阈值和稳定原因发出丢弃事件。
