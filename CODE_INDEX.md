...
| `crates/biliup-cli/src/server/common/timestamp_repair.rs` | 上传前的时间戳异常检测与两级修复，每一步都以复检为准，进程层面失败一律降级直传。第 1 级是 `-c copy` + `setts` bsf 把 packet 时间戳夹成单调递增（只动容器元数据，payload 不解码）；第 2 级才是保画质重编码，它有自己的 10 分钟上限，超时按 `Unfixable` 收口（原片直传 + 本地留档 + 告警），不让 attempt 层的 preprocess watchdog 把整个 attempt 杀掉。 | `normalize_timestamps`、`RepairOutcome`、`FfmpegRunner`、`SystemFfmpeg`、`SETTS_MONOTONIC`、`REENCODE_TIMEOUT` |
| `crates/biliup-cli/src/server/common/ffmpeg_scan.rs` | 全片扫描类 ffmpeg 调用的 stderr 消费：边读边匹配时间戳异常并保留 64KiB 业务解析尾部，同时独立维护有界脱敏诊断附件；spawn/read/wait/非零退出都按调用方固定 stage 发原生失败，只有系统给出时才带退出码。可选 tee 保留 remux/reencode/hook 原先继承的 stderr；read_until 的单行业务临时缓冲尚无硬限。 | `run_scanning_stderr`、`ScanObserver`、`StderrScan`、`stderr_indicates_anomaly` |
