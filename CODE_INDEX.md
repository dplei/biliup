...
| `crates/biliup-cli/src/server/common/upload.rs` | 编排直播分段上传、会话级幂等投稿协调、缺失补传、attempt lease/watchdog（分阶段计时 + 传输阶段滑窗判速）、线路决策入口以及远端结果落库；滑窗判慢中止使用独立慢速失败类别，聚合诊断只承诺最近确认分块；两个服务端入口共用预上传瞬时网络重试，分段成功会在事务外唤醒有 durable 投稿意图的父会话。投稿未指定接口时先走 App，只有结构化 21566 明确拒绝才在同一 attempt 降级一次 Web；显式接口严格、非法配置不发请求。明确拒绝走 60 s 起的普通退避，传输/响应不确定则持久化 `unknown_remote_result` 并保留 claim，禁止自动重投。 | `process_with_upload`、`reconcile_session_submission`、`submit_to_bilibili`、`SubmitFailure`、`preserve_unknown_submission`、`spawn_session_submission`、`persist_segment`、`decide_upload_line`、`pre_upload_with_retry`、`retry_pre_upload_requests`、`upload_enrolled_with_watchdog`、`claim_manual_recovery`、`run_claimed_recovery`、`stop_missing_segment_attempt`、`segment_part_title` |
...
| `crates/biliup-cli/src/server/common/submission_scheduler.rs` | 数据库驱动的待投稿会话启动/周期扫描：只选有持久投稿意图、无 claim 且已到期的会话，以有界并发唤醒统一协调器并分类记录结果；`ok_no_aid`、`submitting`、`unknown_remote_result` 即使异常丢了 claim 也防御性排除，避免重复投稿。 | `start_submission_reconciliation_scan`、`scan_due_submissions`、`due_submission_session_ids` |
...
| `crates/biliup-cli/src/server/common/upload_session.rs` | 维护投稿会话恢复（场次键优先、时钟窗口兜底）、单调持久投稿意图与退避时间，并以生命周期账本检查完整性、确定性重建分 P 和原子 claim/finalize；下播后 `BLOCKED_RECHECK_INTERVAL` 内仅有在飞行（pending/uploading）的阻塞判为 quiet，不写 `blocked_signature`；自动路径只把关闭态零基线会话终结为 `discarded_empty`，人工路径可在无在飞分段、无投稿 claim/远端身份/不确定结果时原子终结为 `discarded` 并保留账本。 | `SessionCompleteness`、`request_session_submit`、`session_submit_readiness`、`discard_empty_session`、`discard_session`、`schedule_submit_retry`、`mark_submit_anomaly`、`hold_session_submit`、`select_recovery_candidate`、`reusable_streamer_info`、`touch_session_activity`、`claim_complete_session`、`BLOCKED_RECHECK_INTERVAL` |
