...
| `crates/biliup-cli/src/server/common/upload_session.rs` | 维护投稿会话恢复（场次键优先、时钟窗口兜底）、单调持久投稿意图与退避时间，并以生命周期账本检查完整性、确定性重建分 P 和原子 claim/finalize；下播后 `BLOCKED_RECHECK_INTERVAL` 内仅有在飞行（pending/uploading）的阻塞判为 quiet，不写 `blocked_signature`；关闭态零基线会话在同一事务中逻辑终结为 `discarded_empty`。`mark_submit_anomaly` 可在远端结果不确定时保留 claim 并清空自动重试时间，空会话清理也拒绝越过该状态。 | `SessionCompleteness`、`request_session_submit`、`session_submit_readiness`、`discard_empty_session`、`schedule_submit_retry`、`mark_submit_anomaly`、`hold_session_submit`、`select_recovery_candidate`、`reusable_streamer_info`、`touch_session_activity`、`claim_complete_session`、`BLOCKED_RECHECK_INTERVAL` |
