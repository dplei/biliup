...
| `crates/biliup-cli/src/observe/external.rs` | 外部进程的有界诊断经**本次调用的采集器**直接写出（附件走不了 tracing 字段），只取当前 dispatch 上的采集器，不搜索全局运行：命令失败发 `processing.command_failed`；exit 0 但时间戳异常措辞无法解析时发 `processing.diagnostic_captured`，两者都把脱敏附件按 event_uid 关联而不复制第三方输出到字段。也为 danmaku/封面/非命令 hook 等没有附件的失败发 auxiliary 事件。 | `command_failed`、`timestamp_anomaly_unparsed`、`auxiliary_failed` |
...
| `crates/stream-gears/src/uploader.rs` | Python 上传参数到上传/封面/投稿的编排，显式传递同一 task，复用服务端线路及持久限流准入。 | `StudioPre`、`upload` |
...
| `crates/biliup/src/downloader/httpflv.rs` | HTTP-FLV 拉流与逐 tag 解析：按关键帧刷盘并在关键帧边界切分段，维护 onMetaData/序列头缓存，用可配置的单次 chunk 读超时（停顿看门狗，默认 30s）兜住码流停顿，断连时产出连接寿命、静默时长、分段进度与媒体时间戳的诊断。**切片关闭原因必须在计数复位之前取值**（否则恒为 `Unknown`）；**本段计时起点只在首个关键帧锚定一次**（不能用 `prev_timestamp == 0` 推断初始化，重发的 timestamp=0 Script tag 会把起点推到当前，定时分段静默失效）；**写出前统一套一层 offset 重基**（`TimestampRebase`：连续性判据按 tag 类型分开维护，修正量按槽持有；候选新基准要连续两个样本才确认，否则 CDN 逐帧交替重发会把时间轴压成每帧 10ms；某槽确认后**不清兄弟槽的判据**，只把新 offset 挂到兄弟的 `adopt` 上、等它自己也偏离旧基准时采纳——否则成对重发的 timestamp=0 垃圾 tag 会把仍在旧基准的兄弟流拖跳几千秒，issue #62；**首次偏离的 tag 紧跟本槽 `last_emit + 1`，不接到全局 `high_water`**，且**确认新基准要求第二个样本严格推进**——B 帧推流的 video 比 audio 晚到 1s 以上，一个重发的 sequence header 接到 audio 的位置会把随后 1s 的真实帧压成 1ms 间隔（上传侧判 Unfixable），一对相同 timestamp=0 的 header 凑成确认则会把 audio 拉到 video 的位置、整段音画错位；**script tag 不走连续性判据**，贴在 `high_water` 上——切段 onMetaData 相邻两个隔整段时长，走判据必然写成「上一个 +1ms」，文件起点被拉回一整段，issue #75），切段重发的 prelude 用当前时间戳（`restamp`）；DTS 基准跳变在源端按分段汇总（首条 1:1，其余计数/首末/极值）。 | `parse_flv`、`Connection`、`Connection::with_stall_timeout`、`DEFAULT_STALL_TIMEOUT`、`read_frame`、`ConnectionDiagnostics`、`FlvProgress`、`download_with_context`、`DtsBackwardRollup`、`TimestampRebase`、`restamp` |
...
| `crates/biliup/src/downloader/util.rs` | 分段判据 `Segmentable`（时间/大小任一超限即切）与录制文件生命周期 `LifecycleFile`；时间判据是**连续前向增量的累加**而不是减法（`MAX_STEP` 以内的推进才计入）：媒体时间轴不保证单调，CDN 重发的 timestamp=0 tag 曾让 `current - start` 瞬间得出小时量级并切出秒级碎片（issue #35）。与当前基准不连续的时间戳先进 `pending_base`，其上再出现一个连续时间戳才落实换基准，**任何一次累加都 ≤ MAX_STEP**。**文件创建时分配稳定 `SegmentIdentity`**（关闭回调与登记账本都用它）。`create()` 撞名（目标或 `.part` 已存在）时追加 `-<n>` 序号：模板只精确到秒，同秒切段曾被 `rename` 静默覆盖（issue #88）。`recording.segment_created/closed` 的发射抽成公开函数，外部进程产出文件的下载器共用同一套字段。 | `Segmentable`、`Segmentable::needed`、`elapsed_time`、`set_time_position`、`set_start_time`、`LifecycleFile`、`LifecycleFile::create`、`SegmentIdentity`、`RecordingOwner`、`allocate_id`、`segment_created`、`segment_closed`、`segment_close_failed`、`close_reason_code`、`SegmentCloseReason` |
...
| `app/(app)/uploads/page.tsx` | 上传列表页：每个分段从登记起就在这里，首传与失败重试同表；独立轮询待投稿会话和分段列表，按状态在单个「详情」列展示进度/去向/错误，并触发会话恢复、会话丢弃（不投稿）、立即上传、换线重传、停止、删除与本场补扫。 | `UploadList`、`renderDetail`、`AttemptHistoryPanel` |
...
| `app/ui/logevents/ProgressView.tsx` | 运行进度：复用 `/v1/status`、`/v1/uploads/missing`、`/v1/uploads/sessions/pending` 三个业务快照，无已知总量只显示阶段，过期快照单独标注，`last_error` 只按错误诊断显示，补传入口共用带按钮外观的导航链接。 | `ProgressView`、`workerState`、`RecoveryLink` |
...
| `crates/biliup-cli/src/server/common/download.rs` | 录制主流程与分段事件处理：拉流、断线重试、切片校验，用有效/可恢复短媒体标记 productive attempt，把有效分段登记后交给上传管道，并在尾段 durable enrollment 后持久关闭会话投稿意图；校验淘汰统一经过同步等待的 `remove_invalid_segment`，删除成功后携带录制/分段身份发出原生丢弃事件。**拉流与分段处理必须 `join` 并发**，校验/合并走 `spawn_blocking`：处理期间不 poll 拉流，socket 约 20s 没人读，抖音 CDN 会断开连接，每次切分丢约 20s（issue #79）。 | `start_download_workflow`、`DownloadTask`、`SegmentEventProcessor`、`remove_invalid_segment`、`persist_closed_session_intents` |
| `crates/biliup-cli/src/server/common/route_health.rs` | 每场录制的拉流线路状态机：按签名无关的 `RouteKey` 累计失败、认证刷新、冷却与稳定尝试；全部冷却时每 `HALF_OPEN_INTERVAL` 半开探测冷却最早到期的一条（探测失败直接重新熔断以轮转），直播间报下播时 `reset_after_offline` 清空线路记录（issue #83）；稳定或有媒体产出的 attempt 会复位旧失败串，但只有原稳定判据进入稳定指标；状态机再从平台刷新后的候选中轮换健康线路，是否启用切线由调用端传入。 | `RouteKey`、`RouteHealthState`、`observe_live_attempt`、`select_route`、`RouteSelection`、`reset_after_offline` |
...
| `crates/biliup-cli/src/server/common/upload.rs` | 编排直播分段上传、会话级幂等投稿协调、缺失补传、attempt lease/watchdog（分阶段计时 + 传输阶段滑窗判速）、线路决策入口以及远端结果落库；滑窗判慢中止使用独立慢速失败类别，但另有线路正因慢冷却时判为整机出口限速、本次 attempt 锁定为不中止且豁免总时长上限（issue #82），聚合诊断只承诺最近确认分块；两个服务端入口共用预上传瞬时网络重试，分段成功会在事务外唤醒有 durable 投稿意图的父会话。投稿未指定接口时先走 App，只有结构化 21566 明确拒绝才在同一 attempt 降级一次 Web；显式接口严格、非法配置不发请求。明确拒绝走 60 s 起的普通退避；内容类拒绝码（`FINAL_REJECTION_CODES`，21588）或远端拒绝连续 `MAX_SUBMIT_REJECTIONS` 次则停止自动重投（`held`，issue #76），本地前置失败不计入；传输/响应不确定则持久化 `unknown_remote_result` 并保留 claim，禁止自动重投。v2 分段失败按 `retry_delay_for_attempt` 退避，时间戳 `Unfixable`（错误带 `UNFIXABLE_UPLOAD_ERROR`）一次用完自动重试次数。 | `process_with_upload`、`reconcile_session_submission`、`submit_to_bilibili`、`SubmitFailure`、`stops_auto_submit`、`fail_enrolled_attempt_with_outcome`、`preserve_unknown_submission`、`spawn_session_submission`、`persist_segment`、`decide_upload_line`、`pre_upload_with_retry`、`retry_pre_upload_requests`、`upload_enrolled_with_watchdog`、`claim_manual_recovery`、`run_claimed_recovery`、`stop_missing_segment_attempt`、`segment_part_title` |
| `crates/biliup-cli/src/server/common/upload_rate_gate.rs` | 维护服务端进程级预上传节流、601 持久冷却和冷却结束后的单探测状态；每次真实预上传请求都必须单独准入并在返回后收口。 | `before_pre_upload`、`record_success`、`record_rate_limited`、`record_non_rate_limit_failure`、`UploadGateState` |
...
| `crates/biliup-cli/src/server/common/upload_line_selection.rs` | 全仓唯一的上传线路决策：纯函数规划（配置/手动优先、冷却回退、auto 兜底）加一步 probe 解析。probe **优先只在 `RECOVERABLE_LINES` 里挑**——那是实测能凭原始 `X-Upos-Auth` 把源对象 GET 回来的线路，而「事后能取回原片」是删本地原片的前提；这些线路全不可用时放开限制重探并 warn，因为传不上去比失去取回通道更严重。显式配置/手动线路只绕过 `probe_failure` 冷却，真实传输失败仍回退。 | `plan_upload_line`、`resolve_planned_line`、`RECOVERABLE_LINES`、`LinePlan`、`LineSource`、`cooling_lines` |
| `crates/biliup-cli/src/server/common/recovery_scheduler.rs` | 到期补传的主动扫描循环与后台执行：按会话串行、按 `segment_order` 顺序领取，接口只负责 claim。自动扫描只取已到期且 `attempts < MAX_AUTO_UPLOAD_ATTEMPTS` 的行；人工「恢复会话」越过退避与次数上限（issue #76）。 | `start_due_recovery_scan`、`recover_due_segments`、`spawn_claimed_recovery` |
| `crates/biliup-cli/src/server/common/submission_scheduler.rs` | 数据库驱动的待投稿会话启动/周期扫描：只选有持久投稿意图、无 claim 且已到期的会话，以有界并发唤醒统一协调器并分类记录结果；`ok_no_aid`、`submitting`、`unknown_remote_result` 即使异常丢了 claim 也防御性排除，避免重复投稿；`held`（远端拒绝后停止自动重投）同样排除，只等人工恢复。 | `start_submission_reconciliation_scan`、`scan_due_submissions`、`due_submission_session_ids` |
...
| `crates/biliup-cli/src/server/common/upload_session.rs` | 维护投稿会话恢复（场次键优先、时钟窗口兜底）、单调持久投稿意图与退避时间（远端拒绝后 `hold_session_submit_after_rejection` 置 `held`、就绪预检返回 `Held`；人工恢复 `rearm_session_submit` 越过退避、清零计数、解除 `held`），并以生命周期账本检查完整性、确定性重建分 P 和原子 claim/finalize；下播后 `BLOCKED_RECHECK_INTERVAL` 内仅有在飞行（pending/uploading）的阻塞判为 quiet，不写 `blocked_signature`；自动路径只把关闭态零基线会话终结为 `discarded_empty`，人工路径可在无在飞分段、无投稿 claim/远端身份/不确定结果时原子终结为 `discarded` 并保留账本。 | `SessionCompleteness`、`request_session_submit`、`session_submit_readiness`、`discard_empty_session`、`discard_session`、`schedule_submit_retry`、`mark_submit_anomaly`、`hold_session_submit`、`select_recovery_candidate`、`reusable_streamer_info`、`touch_session_activity`、`claim_complete_session`、`BLOCKED_RECHECK_INTERVAL` |
...
| `crates/biliup-cli/src/server/common/upload_line_health.rs` | 分类上传网络错误并持久维护单线路失败次数、冷却和到期单探测租约；AUTO 探测失败单列 `Probe`，让显式选路可以只绕过这种误判；成功路径同时记录吞吐 EWMA，实测低于全库基线 1/4 的线路按 30 分钟短冷却挂起；watchdog 主动中止的 `SlowTransfer` 复用同一时长，使冷却覆盖 10 分钟后的首次补传；其它线路的慢冷却同时充当「整机限速」证据。 | `UploadFailureKind`、`acquire_line`、`record_failure`、`record_success`、`machine_wide_slowness`、`SLOW_THROUGHPUT` |
...
| `crates/biliup-cli/src/server/common/timestamp_repair.rs` | 上传前的时间戳异常检测与单级修复：`-c copy` + `setts` bsf **按流累加增量**重写 packet 时间戳（`delta_setts`：增量为负或超过 `MAX_PACKET_STEP_MS` 都压成 1ms，之后按真实增量走），回退与前跳一个表达式都修，内容一帧不丢、时长等于真实内容；只动容器元数据、payload 不解码。曾经的 `max()` 夹取 + 10 秒回退量闸门（`MAX_REPAIRABLE_BACKWARD_MS`）已随之删除——夹取会把大回退压成帧风暴，对前跳结构上无效。修复以复检为准，检测/remux/复检的进程失败分别返回带稳定原因的 `Fallback` 并直传；复检仍异常才是 `Unfixable`，**`upload.rs` 对它报错不上传、本地文件保留**（issue #62 起，此前是原片直传 + 告警）。**文本判据判干净之后还要过一遍 ffprobe 包时间戳数值判据**（`detect_packet_jump`，只在这一步跑，健康文件因此多付一遍解复用）：单调大跳变与**跨流起点差**超过 `MAX_PACKET_STEP_MS` 都报 `Anomalous { max_backward_ms: None }` 进修复；setts 首包 DTS 超过阈值即拉回 0（ffmpeg 已先减掉文件起点，这等于「本流比最早的流晚了一次跳变」），remux 只保留 `0:v:0?`/`0:a:0?`，不带 script 数据流（issue #75）；ffprobe 本身跑不起来时跳过该判据而不是升级成 `DetectFailed`。 | `normalize_timestamps`、`RepairOutcome`、`RepairFallbackReason`、`Detection`、`delta_setts`、`FfmpegRunner`、`SystemFfmpeg::with_context` |
| `crates/biliup-cli/src/server/common/ffmpeg_scan.rs` | 全片扫描类 ffmpeg 调用的 stderr 消费：边读边匹配时间戳异常并保留 64KiB 业务解析尾部，同时独立维护有界脱敏诊断附件；spawn/read/wait/非零退出按调用方 stage 发命令失败，exit 0 且所有命中行都解不出回退量时只收集这些命中行并发 `processing.diagnostic_captured`。`ScanObserver` 可接显式 owned context复用上传身份。异常行顺带解出**最大单次回退量**（认 muxer 两种措辞，未知必须保守处理）；另有 `PacketJumpScan` 不看措辞，按流扫描 ffprobe 包时间戳的大幅前跳，并给出跨流起点差（`start_skew_ms`，issue #75：各流单调、只是起点错开，流内判据全看不见）。可选 tee 保留旧 stderr；read_until 的单行业务临时缓冲尚无硬限。 | `run_scanning_stderr`、`run_packet_jump_scan`、`ScanObserver`、`StderrScan`、`PacketJumpScan`、`stderr_indicates_anomaly`、`parse_backward_ms` |
...
| `crates/biliup-cli/src/server/common/missing_segment.rs` | 分段补传状态机：入队、按状态计数与 stale-uploading 健康快照、后台自愈租约（先取消进程内 attempt 再落库）与 v1 重试状态转换；`MAX_AUTO_UPLOAD_ATTEMPTS` 与 `retry_delay_for_attempt` 是 v1/v2 共用的自动重试上限与退避表。`upload_missing_segment` 的 v1 行是失败/补救队列，v2 行是所有已验证分段的持久账本；当前健康以 `status` 为准，`last_error` 只存真实失败诊断，成功转换必须清空它；`pending`、`failed`、`source_missing` 可进入幂等文件清理与删除 claim。 | `missing_segment_health`、`recover_stale_upload_attempts`、`start_stale_attempt_recovery`、`claim_missing_segment_for_delete`、`enqueue_pending_segment`、`mark_retry_success` |

## 上传核心

| 文件 | 主要作用 | 关键符号 |
| --- | --- | --- |
| `crates/biliup/src/uploader/line.rs` | 定义 B 站上传线路、健康过滤后的自动探测、pre-upload 与服务端确认分块进度。POST 探测体服从服务端 `probe.post`（缺省 0.1 MB、最多 10 MB），避免并发测速用固定 10 MB 抢满中小带宽。`Parcel::recovery()` 取走本次 preupload 的 UPOS 取回描述符，必须在 `upload()` 消耗 parcel 之前调用。探测的候选过滤是纯函数 `retained_lines`：先剔冷却，再在 `allowed` 非空时限定子集。 | `Line`、`Probe::probe_excluding`、`Probe::probe_filtered_with_failures`、`probe_post_bytes`、`retained_lines`、`Parcel::upload_with_observer`、`Parcel::recovery`、`UploadProgress` |
...
| `crates/biliup/src/uploader/bilibili.rs`、`crates/biliup/src/error.rs` | App、Web、BCut 三条投稿请求及公共响应结构；非零投稿响应保留为 `SubmitRejected { code, message }`，让上层只对明确拒绝做安全路由，不再解析 Debug 字符串。传输与响应解析错误保持原类型。 | `BiliBili::submit_by_app`、`BiliBili::submit_by_web`、`BiliBili::submit_by_bcut_android`、`ResponseData`、`Kind::SubmitRejected` |
...
| `scripts/timestamp_shift.py` | 算出把回退的时间戳「平移」接回去所需的 setts 参数并打印现成的 ffmpeg 命令：逐流找回退点、按典型帧间隔补偏移、多次回退累加。服务端已改成增量模型（`timestamp_repair.rs` 的 `delta_setts`）后两者效果相当，本脚本留给本机应急与服务端修不好的存量片；只算回退，前跳形态尚未覆盖。`segment-recover` skill 用它。 | `find_drops`、`expression`、`typical_delta` |
...
- `crates/biliup-cli/src/server/common/upload.rs` → `crates/biliup-cli/src/server/common/upload_rate_gate.rs`、`crates/biliup/src/uploader/line.rs`（`pre_upload_with_retry`、`before_pre_upload`、`Line::pre_upload`）：两个服务端上传入口共用 typed 瞬时网络重试；每次真实预上传请求都单独经过并收口全局 gate，预上传失败不再污染所选 UPOS 线路的 breaker。
...
- `app/(app)/uploads/page.tsx` → `crates/biliup-cli/src/server/api/endpoints.rs` → `crates/biliup-cli/src/server/common/upload_session.rs`（`get_missing_uploads`、`get_pending_submit_sessions`、`recover_session_uploads`、`discard_empty_upload_session`、`discard_session`）：页面分别读取分段聚合视图和独立待投稿会话；不确定 claim 不显示危险操作，其他已关闭且无在飞分段的会话可原子丢弃为不投稿，保留会话与分段账本。
...
- `crates/biliup-cli/src/server/common/download.rs` → `crates/biliup-cli/src/server/common/upload_session.rs`（`request_session_submit`）：尾段均已 durable enrollment 后先持久化「本场已关闭、最终必须投稿」，再关闭上传 channel，避免上传尾段期间退出时丢失目标状态。首次写入意图时同步用 `hold_session_submit` 把 `next_submit_at` 置为 `close + config.delay`：录制循环的宽限期只管「还没判下播」，这个窗口管「判了下播但 delay 内又开播」——会话不 finalize，`reusable_streamer_info` / `find_or_create_session` 才能把重连分段续接进来而不是拆成第二稿（issue #60）。
...
- `crates/biliup-cli/src/server/common/upload.rs` → `crates/biliup-cli/src/server/common/upload_line_health.rs`（`baseline_mbps`、`record_success`）：watchdog 在进入传输阶段时读一次全库吞吐基线并缓存，`classify_transfer_rate` 用它判「在动但在爬」；上传成功回写的 EWMA 又是下一次的基线来源，两条慢判据共用同一个 `SLOW_RATIO`。判慢时再经 `active_cooldowns` + `machine_wide_slowness` 查别的线路是否也因慢被冷却，有则忍住不中止——换线无益时不作废已传字节。
...
- `crates/biliup-cli/src/server/common/download.rs` → `crates/biliup-cli/src/observe.rs`（`RecordingIdentity`、`retry_scheduled`、`segment_enrolled`、`segment_discarded`）：录制循环在刷新下一轮流之前保存已结束 attempt，断连后的 retry 与它共享 DA；分段处理器复用同一身份，登记和校验删除结果也逐一映射为原生事件。
...
- `crates/biliup/src/downloader/live/douyin.rs` → `crates/biliup-cli/src/server/common/download.rs` → `crates/biliup-cli/src/server/common/route_health.rs`（`build_stream_candidates`、`DownloadTask::execute`、`select_route`）：抖音检查把真实响应候选交给录制循环，循环在确认仍开播后先记上一 attempt 的线路结果，再用刷新候选选择下一条线路；平台开关由录制循环组装。
- `crates/biliup-cli/src/server/common/download.rs` → `crates/biliup-cli/src/server/common/util.rs`（`FileValidator::validate`）：分段关闭后按同一份判据分流——`Invalid` 直接删除，`RecoverableShort` 仅在`preserve_recoverable_short_segments` 开启时进入合并管线，否则同样删除；进入管线后由 `plan_short_segment_flush` 分流：pending 只有一个时直接 `enqueue_validated` 作为独立分 P；多个则按参数分组，同组 `>1` 才 `merge_compatible_segments`，落单的组走 `defer_recovery_batch`（该表没有消费者，只能人工处理）。`merge_compatible_segments` 必须把工作目录绝对化：默认文件名含 `HH:MM:SS`，相对 concat 列表名会被 ffmpeg 当成 URL scheme，且直接复制和 remux concat 共用这一路径。
...
- `crates/biliup-cli/src/server/common/upload.rs` → `crates/biliup-cli/src/server/common/audio_normalization.rs`、`timestamp_repair.rs`（`normalize_for_upload`、`normalize_timestamps`）：预处理顺序是先标准化、后时间戳检测，检测的对象是标准化产物而不是原片。标准化的测量遍已经完整 demux 过原片并顺带扫了时间戳，原片干净时 `upload_single_file_with_repair` 跳过对产物的整片扫描；诊断缺失或原片异常时照常走完整的检测/修复链路。判据抽在 `timestamp_repair_decision`：**归一化没产出就没有「源已验干净」这个结论可用**，`Original` 的每一个 reason 都必须扫（有回归测试锁住，别给它补乐观默认）。两个系统 runner 都从本次 `UploadIdentity` 构造显式 context，`processing.command_failed` 与完成事件按同一分段/attempt 关联。
- `crates/biliup-cli/src/server/common/ffmpeg_scan.rs` → `crates/biliup-cli/src/observe/external.rs`（`timestamp_anomaly_unparsed`）：扫描成功但所有异常命中行都无法解析时，把仅含这些行的有界脱敏附件连同显式上传身份直接交给当前采集器。
...
- `crates/biliup-cli/src/server/api/endpoints.rs` → `crates/biliup-cli/src/server/common/missing_segment.rs`（`missing_segment_health`）：健康接口按 `status` 实时计数 + 识别尚未被 60 秒自愈周期收敛的 stale uploading 行，不以 `last_error` 是否非空判健康。
