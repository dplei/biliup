...
| `crates/biliup-cli/src/server/common/upload_line_health.rs` | 分类上传网络错误并持久维护单线路失败次数、冷却和到期单探测租约；成功路径同时记录吞吐 EWMA，实测低于全库基线 1/4 的线路按 30 分钟短冷却挂起；watchdog 主动中止的 `SlowTransfer` 复用同一时长，使冷却覆盖 10 分钟后的首次补传。 | `UploadFailureKind`、`acquire_line`、`record_failure`、`record_success`、`SLOW_THROUGHPUT` |
