...
| `crates/biliup-cli/src/server/common/ffmpeg_scan.rs` | 全片扫描类 ffmpeg 调用的 stderr 消费：边读边匹配时间戳异常并保留 64KiB 业务解析尾部，同时独立维护有界脱敏诊断附件；spawn/read/wait/非零退出都按调用方固定 stage 发原生失败，只有系统给出时才带退出码。`ScanObserver` 可接调用方显式 owned context，直接 emitter 因而能复用上传身份，无身份时仍只带扫描文件名。命中的异常行顺带解出**最大单次回退量**（认 muxer 的两种措辞，解不出返回 `None`——调用方必须当作「未知」保守处理，不能当作 0），时间戳修复的闸门靠它决策。另有一条**不看措辞只看数值**的判据：`PacketJumpScan` 流式消费 ffprobe 的 `stream_index,dts_time`，按流分别比对相邻包，前跳超过 `MAX_PACKET_STEP_MS`（30s，与录制侧 `REBASE_MAX_STEP_MS` 同源）即判异常——32 位倒退被解复用器展开成单调大跳变时 muxer 一条警告都不打，这是确定性漏检。可选 tee 保留 remux/hook 原先继承的 stderr；read_until 的单行业务临时缓冲尚无硬限。 | `run_scanning_stderr`、`run_packet_jump_scan`、`ScanObserver`、`ScanObserver::with_context`、`StderrScan`、`PacketJumpScan`、`MAX_PACKET_STEP_MS`、`stderr_indicates_anomaly`、`parse_backward_ms` |
