...
import { Layout, Nav, Spin, Typography, Select, Card, Button, Toast, Tabs, TabPane, Tag } from '@douyinfe/semi-ui'
import {
  IconCustomerSupport,
  IconRefresh,
  IconClear,
...
/**
 * 旧的按文件切 tab 的实时日志页，逐字保留：迁移期它是独立于新事件库的另一条证据链，
 * 不能因为新页上线就改它的读取路径或输出。这里只在标题旁标明数据来源。
 */
export default function LegacyLogViewer() {
        ...
        <Nav
          ...
          header={
            <>
              ...
              <Tag size="small" color="grey" style={{ marginLeft: 12 }}>
                数据来源：旧日志文件
              </Tag>
              <Link
                href={LOG_EVENTS_IS_DEFAULT ? '/logviewer' : '/log-events'}
                style={{ marginLeft: 12, fontSize: 13, color: 'var(--semi-color-link)' }}
              >
                {LOG_EVENTS_IS_DEFAULT ? '前往日志与事件' : '试用日志与事件'}
              </Link>
            </>
          }
          mode="horizontal"
