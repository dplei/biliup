...
服务端早先用 `setts=...max(TS, PREV_OUT+1)` 把回退那段时间**夹掉**，代价是回退量一旦
接近剩余内容时长，后面的内容会被压成帧风暴，所以设过 10 秒的闸门。现在服务端已改成
按流累加增量（见 `crates/biliup-cli/src/server/common/timestamp_repair.rs` 的 `delta_setts`），
效果与这里的平移相当；修不好的文件不再直传，留在本地等这类工具处理。

平移：**把回退点之后的时间戳整体加一个偏移**，让时间轴接着往下走。内容一帧不丢，只是
文件变长。这个脚本负责算出每条流的「从第几个包开始加、加多少」。只处理回退，前跳形态
（相邻包前跳几十秒以上）暂未覆盖。
