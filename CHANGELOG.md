...
## 1.3.5
- 更新时间：2026.08.31
- 💡「自动增强录音音量」现在会如实记录**实际达到的响度**。ffmpeg 在原始音轨峰值过满时会
  自动改用动态模式，结果可能达不到设定目标（实测最多差过 9 dB），此前这一步完全没有输出，
  看日志只会以为一切正常。现在处理完成的日志会带上实际响度与所用模式，偏离目标超过 1 dB
  时另记一条说明。**只是记录，不改变任何处理行为**，产物与此前完全一致。

## 1.3.4
- 更新时间：2026.08.31
- 🔧修复「自动增强录音音量」对**非首段**录像完全不生效的问题。分段录像的时间戳沿用整场
  直播的时间轴，一致性校验错把容器自报的末尾时间戳当成时长，于是每一段都被判为「时长对
  不上」而丢弃处理结果、改传原片——白跑一遍满载转码。校验现在比对的是真实内容时长。
- 🔧修复音量样片截取在非首段录像上取到空片段的问题（同一处时长口径，早于 1.3.3 就存在）。
- 💡一致性校验失败时的日志带上实测数值（时长、容差、体积、编码构成），不必再回头
  ffprobe 现场文件才能判断问题出在哪。
- 💡新增熔断：同一条校验连续三次挡下处理结果时，本次运行不再发起后续标准化并打一条
  ERROR 日志，避免确定性故障持续空烧 CPU。上传照常进行，重启即恢复。

## 1.3.3
- 更新时间：2026.08.30
- ⚠️**行为变更**：开启「自动增强录音音量」后，处理结果会**直接替换原始录像**，磁盘上每段
  只保留一份。后处理脚本（postprocessor）与补传拿到的都是处理后的文件，原始音轨不再保留。
  想要旧行为就打开 `audio_normalization_keep_original`，代价是每段占用两份。
- 💡响度标准化新增磁盘保留线 `audio_normalization_disk_reserve_gib`（默认 5 GiB）：可用空间
  放不下一份处理结果时跳过本段直接上传原片；处理途中磁盘跌破这条线会立即中止并回退。
  录制与上传链路不受影响。
- 💡补传不再重复处理已经标准化过的分段，省掉一次多余的有损重编码。
