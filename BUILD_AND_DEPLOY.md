...
| digest 前缀 | tag | 内容 |
|---|---|---|
| `89e2ad4f` | `1.3.0-submitliveness` | **投稿活性修复（7 张票）**：系统此前满足「不完整会话绝不投稿」的安全性，但不满足活性——下播提交检查一旦早于最后一个分段成功就写下 `blocked_missing_segments`，此后没有任何消费者会再次投稿；`submit_state=NULL` 的会话同样无人接手。①**持久投稿意图**：`upload_session` 加 `submit_requested_at`/`next_submit_at`（迁移 `19`）与 `submit_retry_attempts`（迁移 `20`）。前者表达「本场已结束、必须最终投稿」，后者只记最近一次闸门/投稿结果，分工明确；不能用「当前 lifecycle 全 succeeded」直接判断该投稿，因为直播期间两个分段之间也会短暂满足该条件。②**统一协调器** `reconcile_session_submission`：下播、`persist_segment` 成功、人工 recover、启动/周期扫描都只唤醒它，远端请求绝不发生在 SQLite 事务内。③**结束边界落库顺序**：尾段完成 durable enrollment → 写投稿意图 → 关闭上传 channel，因此上传尾段期间进程退出也不会丢失目标状态。④**recover 接口兜底**：先幂等写入人工投稿意图再领取分段，无可领取分段且账本完整时异步唤醒协调器，不再用 `started=[]` 冒充成功。⑤**扫描器** `submission_scheduler`：启动即扫、之后每分钟扫，只选有意图、未 finalized、无 claim 且退避到期的会话，`ok_no_aid` 与 claim-less `submitting` 保守排除。⑥`GET /v1/uploads/sessions/pending` + 补传页独立「待投稿会话」区域，五态（waiting/ready/submitting/retry/manual）文案与按钮各异，不确定状态不提供普通重试按钮。⑦事故回归测试全程无外网。⚠️**含两个迁移**：`19_add_session_submit_intent.sql`、`20_add_submit_retry_attempts.sql`；迁移只回填历史 `blocked_missing_segments`，**刻意不动 `submit_state=NULL`**（可能仍在直播）。部署前已对生产库只读核对：`blocked_missing_segments` **0 行**，故本次回填是空操作，无首启批量投稿风险。验证：`cargo check --workspace`、`cargo test --workspace`、`tsc --noEmit`、`next build` 均通过；**并在本地 dev 环境跑了一场真实抖音直播复现 issue 的竞态**：3 分钟分段 ×5，尾段仍在 transferring 时关闭生产端，`trigger=DownloadClosed` 判 `Blocked{expected:5, succeeded:3}` 且远端零调用，80 秒后尾段成功由 `trigger=SegmentPersisted` 自动投出唯一稿件 `BV1iTtc6vEzH`（`submit_attempts=1`，`is_only_self=1`），分P标题按 `segment_order` 严格递增且取原始录像名；同场还直接观测到「账本暂时完整但无投稿意图时周期扫描零投稿」。构建冒烟：`uname -m`=`x86_64`、`biliup --version`=`biliup-cli 1.3.0`、`ffmpeg` 含 `loudnorm`；推送 `latest` + `1.3.0-submitliveness`，远端 digest=`sha256:89e2ad4f95ca9c23c4abad99cf60d27da14478893015715a9a88cba06e646cb3`。 |
