...
真实缺陷是两个既有时间尺度互相抵消：`slow_transfer` 原先通过
`UploadFailureKind::RequestTimeout` 进入普通失败梯度，首次只冷却线路 1 分钟；生命周期行却要
10 分钟后才能重试。到补传发生时线路已恢复
...
重新唤醒投稿协调器。也不需要新增有界重试；停止重试只会把 `failed` 行永久留下。Step 04 已复用
已有 30 分钟慢线路冷却，让 10 分钟后的首次补传必然避开刚被判慢的线路。
