...
4. `public/config.yaml` 注释、前端说明文案与 `CHANGELOG.md` 都写明「原片会被覆盖、
   postprocessor 收到的是标准化文件」。
...
`CHANGELOG.md` 在「未发布」段记了一条 ⚠️ 行为变更与两条 💡。**发版时把段标题改成实际
版本号**（按 `CLAUDE.md` 的约定，发版前会先 bump `Cargo.toml` 的 PATCH）。
