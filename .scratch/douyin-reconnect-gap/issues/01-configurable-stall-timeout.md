...
当前 `dev` 仍保留完整实现与单测；本 step 不需要继续写代码。下一步只做主播级 8 秒覆写和至少
两条同源有效样本验收。若出现正常码流被误判，再按现有文档上调阈值，不新增自适应策略。

### 2026-09-09 灰度配置前置修复

实际通过主播「配置覆写」写入 8 秒时发现，平台面板会把旧 override 的全部字段灌进表单；提交时
这些未被用户改动的旧值又覆盖顶部 JSON，导致未单独做成控件的字段无法修改。已在共享提交入口
只合并相对现有 override 真正变化的控件值，保留顶部 JSON 的显式修改。待本分支合并发布后再写入
8 秒并开始两场同源验收；当前线上配置未改变。
