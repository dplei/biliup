...
`biliup-observability` crate，验收见 [P0](../../.archive/structured-logging-p0-p2/receipts/P0.md) / [P1](../../.archive/structured-logging-p0-p2/receipts/P1.md)。
...
- HLS 沿用文件层身份，不按 URI 或列表时间补造关联。序列缺口记录片段数量，不连续边界
  关联新输出文件；仅收到非空媒体后可宣告已有连接恢复。P3/14 的终止与采证据前置修复：
  从序号 0 开始、ENDLIST 结束、空直播列表等待、刷新有节制、小数时长参与分段；非法列表
  显式返回错误且不再落原始响应调试文件。错误/取消须能经调用返回与文件关闭事件核对。
  这些修复不启用额外下载器或 HLS 解密/fMP4 能力，具体字段见 contract-v1 的 HLS 增量。
...
P3/14 剩余调用点采用加法式验证：保留旧日志和旧业务执行，在相同边界旁写原生事件；本地只
要求构建、基本启动和新采集关闭时不影响业务，不再为每个外部工具构造正常/错误/取消矩阵。
入口、平台和低频失败的有效性由日常实际运行产生样本后，通过两份独立来源持续对比确认。
这项收敛不追溯取消 P0–P2 已完成的底座/存储故障测试，也不降低 P4 前第一轮完整观察门槛。
...
7. Rust CLI、wheel CLI、Python 下载/上传函数分别保留原生调用点；是否实际触发及结果在
   双写观察中按入口记录，不以本地穷举矩阵作为 P3/14 代码接入前置。P5/P6 无新 `.log`，
   旧文件和现有外部输出文件不自动删除。P2–P4 保留真实旧来源，作为阶段性验收前提。
