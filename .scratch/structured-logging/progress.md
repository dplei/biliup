...
P0–P2 已完成并通过本地受控验收；P3 进行中：12（验收 partial）、13、15、16 均已交付，**14 已完成首批代码，验收 partial**。
...
| [14 全范围覆盖](issues/14-coverage-expansion.md) | P3 | in-progress | partial | [首批回执](receipts/P3.md#任务-14-第一批独立上传入口)：独立上传入口已接入，错误/三态/回退通过；正常远端样本、其余批次与首轮观察待完成 |
...
- 最近一轮仅处理任务 14 的首批独立上传入口，沿用 `refactor/issue2-260831-153007`，
  代码、契约、回执、覆盖清单与索引同批提交。未改页面默认、旧 sink、业务重试或 watchdog。
- Rust/wheel CLI 的命令上传、配置逐稿上传、append 与 Python 上传已有原生 task/attempt
  及输入序号；缺凭据、多次调用、关闭/开启/库不可用三态和关闭回退实跑通过；三份双源包
  complete + 确定性校验 passed。正常远端上传、续传与追加成功仍待本批代码上的运行补验。
- 前置 P0–P2 有效；12、13、15、16 既有交付保持，12 仍缺真实断连/重连样本。
  历史真实录制→预处理→上传→补传→投稿及页面验收见 P3 第二轮回执，不替代新入口样本。
- 14 仍为 in-progress / partial，P3 仍 in-progress / partial / pending。系统/认证/审计/
  外部诊断、页面整场上传、外部下载器/HLS、诊断分类清单及首轮完整观察尚未完成。
  录制期限流事件仍为 `transport`，本批只细化独立上传的 typed 601 错误；不能宣称旧差异已关闭。
- 历史「多次嵌入调用共享run」更正为「重叠调用共享，排空后的顺序调用新建run」，没有修改底座。
- 下一轮入口：`@.scratch/structured-logging/stage-prompts.md 继续 P3，仅处理任务 14`。
  先做页面整场上传的 task 贯通，并用仅自己可见模板补独立入口正常远端样本；每批单独回写。
