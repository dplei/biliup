...
P0–P2 已完成并通过本地受控验收；P3 进行中：12（验收 partial）、13、15、16 均已交付，**14 已接入前六批原生能力、完成未迁移诊断分类清单，并在第八批调整为加法式双日志验证，验收 partial**。
...
| [14 全范围覆盖](issues/14-coverage-expansion.md) | P3 | in-progress | partial | [第八批回执](receipts/P3.md#任务-14-第八批加法式双日志验证策略与工作量重估)：当前只剩外部下载器与 danmaku 异步失败共 3 个源码 gap，预计 2 个实现 session / 3.5–7 小时；本地矩阵改为实际双写自然采样，首轮观察仍待完成 |
...
- 最近一轮仅处理任务 14 第八批：调整剩余验证策略并重估工作量。沿用
  `refactor/issue2-260831-153007`，父提交 `9f2cd39`；本批只改计划/回执，不改业务源码，未 push。
...
- 新策略明确并行的是两套日志证据而不是两套业务进程：剩余实现只在旧边界旁增加失败隔离的
  原生调用，不为日志接入重构子进程、文件搬运或状态机。普通 debug/trace 仍由 bridge 提供，
  不逐行复制成原生事件。wheel/Python、恢复/outbox、真实辅助失败、真实断连和外部工具矩阵
  转为实际运行中的自然样本；未触发项保持待观察，不补造通过。
- 剩余主动实现重估：外部下载器 2–4 小时、danmaku 异步失败 1–2 小时、收口 0.5–1 小时，
  合计约 3.5–7 小时并按仓库规则拆成 2 个实现 session。第一轮双写观察另计日历时间，不计入
  待实现代码。旧 601 `transport` 差异与 FFmpeg 秒级命名碰撞保留为运行中发现项。
- 本批验证：分类漂移检查仍为 62 个文件 / 554 个宏位置 / 3 个 gap，代码索引 109 个文件 /
  56 条关系有效，`git diff --check` 通过。没有业务源码改动，未运行 Rust/前端测试。
- 下一轮入口：`@.scratch/structured-logging/stage-prompts.md 继续 P3，仅处理任务 14`。
  下批只做 Streamlink 与 YtDlp/YtArchive 的加法式原生调用和最低运行门槛；不安装第三方工具、
  不写假命令矩阵、不修现有 stop/文件搬运。danmaku 异步 recorder 失败回传随后单独处理；
  三项源码 gap 闭合后即可开始首轮实际双写观察，但未自然触发的路径仍不能写 passed。
