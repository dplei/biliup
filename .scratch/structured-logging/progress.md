...
P0–P2 已完成并通过本地受控验收；P3 进行中：12（验收 partial）、13、15、16 均已交付，**14 已接入独立上传与页面整场上传，验收 partial**。
...
| [14 全范围覆盖](issues/14-coverage-expansion.md) | P3 | in-progress | partial | [第二批回执](receipts/P3.md#任务-14-第二批页面整场上传)：独立上传及页面 task 已接入，三态/回退通过；页面真实私密投稿通过，脱敏缺口已补修；其余入口样本、覆盖批次与首轮观察待完成 |
...
- 最近一轮仅处理任务 14 第二批页面整场上传，沿用 `refactor/issue2-260831-153007`，父提交
  `afa8b08`，代码与回执同批提交。未改页面默认、旧 sink、业务重试、账本或 watchdog。
- 页面接受响应新增 task；后台跨 spawn 传递上下文，关联每次上传及最终投稿；不从首文件的
  模板匹配结果推断整批录制身份。Rust/wheel HTTP 三态、并发/重复请求、task 查询和回退通过。
- Rust 页面真实合成视频上传与仅自己可见投稿通过，远端只读回查确认私密标志。该样本采集后
  发现桥接/导出的 `ResponseData` 脱敏缺口，已补修并重导；本批最终构建三态和合成脱敏回归
  通过，不拼接成同一最终采集版本的完整观察。详见第二批回执。
- 首批 CLI 命令/配置/append 与 Python 上传的远端正常/恢复样本仍待补验；页面成功不代计。
  12、13、15、16 既有交付保持，12 仍缺真实断连/重连；旧 601 `transport` 差异仍未关闭。
- 14 仍 in-progress / partial，P3 仍 in-progress / partial / pending。其余下载器/HLS、系统/
  认证/审计/外部诊断、分类清单及首轮完整观察尚未完成。历史原始响应包须按新脱敏规则重审，
  本次只重导本批真实成功包；不宣称历史所有对外证据都已修复。
- 下一轮入口：`@.scratch/structured-logging/stage-prompts.md 继续 P3，仅处理任务 14`。
  下批可接其余下载路径（外部下载器/HLS），并单独安排独立上传入口的正常/恢复补验。
