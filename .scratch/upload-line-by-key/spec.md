...
| 步 | 文件 | 状态 |
|---|---|---|
| [01](steps/01-line-explicit.md) | `Line::explicit`，删 15 个构造函数，`explicit_upload_line` 改校验+构造 | 待做 |
| [02](steps/02-drop-enums.md) | 删 `biliup-cli` / `stream-gears` 的 `UploadLine` 枚举与 match，CLI `--line` 改自由字串 | 待做 |
