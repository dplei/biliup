...
- 一次只攒下**一个**短片（典型场景：分段边界后几十秒内断流、随后续录）时没有合并对象，它会按当时的顺序位置直接作为独立分 P 入队上传，不写 `Deferred`。
...
只有合并产物上传成功且视频引用 durable 落库后，后处理才会接触原始短片。`Deferred` 批次不会自动删除原片，**也没有后台任务会自动重试**：`next_retry_at` 只是建议时间，批次需要人工处理（`manual_recover` 或删除）。
